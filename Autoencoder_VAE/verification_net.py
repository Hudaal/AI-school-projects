...
class VerificationNet:
    ...
    def train(self, x_train, x_test, y_train, y_test, epochs: np.int = 10) -> bool:
        """
    ...
    def check_predictability_3C(self, data: np.ndarray, correct_labels: list = None, tolerance: np.float = .8):
        predictions, beliefs = self.predict(data=data)
        all_channels_predictions = []
        for i in range(0, len(predictions), 3):
            one_labels = []
            for step in range(3):
                one_labels.append(predictions[i+step])
            all_channels_predictions.append(one_labels)

        # predictions = predictions[beliefs >= tolerance]

        all_channels_beliefs = []
        for i in range(0, len(beliefs), 3):
            one_labels = []
            for step in range(3):
                one_labels.append(beliefs[i+step])
            all_channels_beliefs.append(one_labels)

        predictability = len(all_channels_predictions) / len(data)
        if correct_labels is not None:
            # Drop those that were below threshold
            sum_ = 0
            for i in range(len(all_channels_predictions)):
                if (all_channels_predictions[i] == correct_labels[i]).all():
                    sum_ += 1
            all_images_count = len(data) / 3
            accuracy = sum_ / all_images_count
        else:
            accuracy = None
        return predictability, accuracy
...
if __name__ == "__main__":
    ...
    net.train(gen.test_images, gen.test_images, gen.train_labels, gen.test_labels, epochs=15)
    ...
    cov = net.check_class_coverage(data=img, tolerance=.5)
