...
class StackedMNISTData:
    ...
    def plot_example(self, images: np.ndarray = None, labels: np.ndarray = None, name='image') -> None:
        """
        ...
        for img_idx in range(no_images):
            ...
            if self.channels == 1:
                ...
                plt.imsave('images/original1_{i}.png'.format(i=img_idx), images[img_idx, :, :, 0].reshape(28, 28))
            else:
                plt.imshow(images[img_idx, :, :, :].astype(np.float))
                plt.imsave('images/original3_{i}.png'.format(i=img_idx), images[img_idx, :, :, :].astype(np.float))
            plt.xticks([])
            plt.yticks([])
            plt.title(f"Class is {str(labels[img_idx]).zfill(self.channels)}")

        # Show the thing ...
        fig1.savefig('images/{text}.png'.format(text=name))
        plt.show()


if __name__ == "__main__":
    gen = StackedMNISTData(mode=DataMode.COLOR_BINARY_MISSING, default_batch_size=1)
    img, cls = gen.get_random_batch(batch_size=8)
    print(img[0].shape)
    print(img.shape)
    print(img[0][:, :, 0].shape)
    print(img[0][:, :, 0])
    print(np.expand_dims(img[0][:, :, 0], axis=2))
    print(np.expand_dims(img[0][:, :, 0], axis=2).shape)
    all_ = np.array([np.expand_dims(img[0][:, :, 0], axis=2), np.expand_dims(img[0][:, :, 1], axis=2),
                     np.expand_dims(img[0][:, :, 2], axis=2)]).reshape(1, 28, 28, 3)
    print(all_.shape)
    print(cls)
    plt.imshow(np.expand_dims(img[0][:, :, 0], axis=2), cmap="binary")
    plt.show()
    plt.imshow(np.expand_dims(img[0][:, :, 1], axis=2), cmap="binary")
    plt.show()
    plt.imshow(np.expand_dims(img[0][:, :, 2], axis=2), cmap="binary")
    plt.show()
